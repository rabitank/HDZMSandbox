...
UHEmitterComponent::UHEmitterComponent()
{
	...
	auto it  = EmitterCores.begin();

	EnergySourceComp = nullptr;
...
void UHEmitterComponent::BeginPlay()
{
	...
	//init EnergySource
	AHPlayerCharacter* playerPawn = Cast<AHPlayerCharacter>(GetOwner());
	if (playerPawn != nullptr)
	{
		EnergySourceComp = UHAttributeComponent::GetAttribute(playerPawn);
	}
...
void UHEmitterComponent::AddCore(AActor* Instigator, TSubclassOf<UHEmitterCoreBase> CoreClass)
{
	...
	if (ensure(CoreIns))
	{
		CoreIns->Initialize(EnergySourceComp, this);
...
bool UHEmitterComponent::StopCurrentCoreEmit(AActor* Instigator)
{
	if (CurrentCore->IsShooting())
	{
		CurrentCore->StopShoot(Instigator);
		return true;
	}
...
void UHEmitterComponent::EmitCurrentBullet(AActor* instigator)
{
	

	//Execute_InterfaceMethodName; generated by ue;
	if (ensure(CurrentCore))
		if (CurrentCore->CanEmit(instigator))
		{
			IHEmitInterface::Execute_Emit(CurrentCore, instigator);
		}
		else
		{
			FString msg = FString::Printf(TEXT("%s: Emitter Emitting filed: Current core Can't emit"), *GetNameSafe(instigator));
			LogOnScreen(this, msg,FColor::Yellow) ;

		}
	else
	{
		FString msg = FString::Printf(TEXT("%s: Emitter Emitting filed: Current core is Nullptr"), *GetNameSafe(instigator));
		LogOnScreen(this, msg,FColor::Yellow) ;
	}
}
...
void UHEmitterComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	//{
		FColor TextColor = CurrentCore->IsShooting() ? FColor::Green : FColor::Blue;
