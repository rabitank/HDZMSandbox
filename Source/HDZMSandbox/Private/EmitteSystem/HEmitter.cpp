...
void AHEmitter::UpdateEmitterBehaviour()
{
	...
	FVector SmoothedTargetPivotLocation = CaculateAxisIndepentLag(SmoothPivotLocation, PivotTransform.GetLocation(), PivotTransform.Rotator());
	SmoothedTargetPivotLocation += UKismetMathLibrary::GetForwardVector(PivotTransform.Rotator()) * GetEmitterCurveValue(Curve_PivotXOffset)
...
void AHEmitter::BeginPlay()
{
	Super::BeginPlay();
	 ...
	 UHEmitterAnim* emitterAnim = Cast<UHEmitterAnim>(ComEmitterMesh->GetAnimInstance());
	 if (ensure(emitterAnim))
	 {
		 AHPlayerCharacter* player = Cast<AHPlayerCharacter>(GetInstigator());
		 if (ensure(player))
		 {
			 emitterAnim->ControledCharacter = player;
			 EnableInput(Cast<APlayerController>(player->GetController())); //接受输入/输入会传播给Emitter;
		 };
		 emitterAnim->OwningEmitter = this;
	 }
